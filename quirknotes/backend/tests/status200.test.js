...
test("/getAllNotes - Return list of two notes for getAllNotes", async () => {
    for (let i=1; i<3; i++){
        const postNoteRes = await fetch(`${SERVER_URL}/postNote`, {
            ...
            body: JSON.stringify({
            title: "NoteTitleTest"+i,
            content: "NoteTitleContent"+i,
            }),
        });
    };
...
test("/patchNote - Patch with content and title", async () => {
    ...
    const patchNoteRes = await fetch(`${SERVER_URL}/patchNote/${postNoteBody.insertedId}`,{
        ...
        body: JSON.stringify({
            title: "NewNoteTitleTest",
            content: "NewNoteTitleContent",
        }),
    });
...
test("/updateNoteColor - Update color of a note to red (#FF0000)", async () => {
    ...
    const updateNateColorBody = await updateNoteColorRes.json();
    expect(updateNoteColorRes.status).toBe(200);
    expect(updateNateColorBody.message).toBe('Note color updated successfully.');
});